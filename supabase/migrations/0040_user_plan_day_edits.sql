-- 0040: User-owned plan-day workout edits.
-- A plan edit clones the current workout instead of mutating shared catalog data.
...
DECLARE
  ...
  total_seconds INT := 0;
BEGIN
  SELECT d.id, d.status, d.workout_id, p.user_id
  INTO day_row
  ...
  IF day_row.status NOT IN ('locked', 'available') THEN
    RAISE EXCEPTION 'Only unopened plan days can be edited';
  END IF;
  ...
  INTO source_workout
  FROM workouts
  WHERE id = day_row.workout_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Source workout not found'; END IF;

  FOR exercise_row IN SELECT value FROM jsonb_array_elements(p_exercises)
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM exercises
      WHERE id = (exercise_row->>'exerciseId')::UUID AND is_active = true
    ) THEN
      RAISE EXCEPTION 'Exercise is not available';
    END IF;
    IF COALESCE((exercise_row->>'sets')::INT, 0) < 1
      OR COALESCE((exercise_row->>'restSeconds')::INT, -1) < 0
      OR ((exercise_row->>'reps') IS NOT NULL AND (exercise_row->>'reps')::INT < 1)
      OR ((exercise_row->>'durationSeconds') IS NOT NULL AND (exercise_row->>'durationSeconds')::INT < 1) THEN
      RAISE EXCEPTION 'Invalid exercise prescription';
    END IF;
    total_seconds := total_seconds
      + COALESCE((exercise_row->>'durationSeconds')::INT, 60) * (exercise_row->>'sets')::INT
      + (exercise_row->>'restSeconds')::INT * GREATEST(0, (exercise_row->>'sets')::INT - 1);
  END LOOP;

  INSERT INTO workouts (name, slug, description, duration_seconds, estimated_calories, is_active, owner_user_id)
  VALUES (
    source_workout.name || ' (Edited)',
    'edited-' || replace(p_plan_day_id::TEXT, '-', '') || '-' || substr(gen_random_uuid()::TEXT, 1, 8),
    source_workout.description,
    GREATEST(60, total_seconds),
    GREATEST(1, round(total_seconds / 60.0 * 8)),
    true,
    auth.uid()
  )
  ...
  LOOP
    ...
    VALUES (
      cloned_workout_id,
      (exercise_row->>'exerciseId')::UUID,
      position,
      (exercise_row->>'sets')::INT,
      NULLIF(exercise_row->>'reps', '')::INT,
      NULLIF(exercise_row->>'durationSeconds', '')::INT,
      (exercise_row->>'restSeconds')::INT
    );
  END LOOP;

  UPDATE user_plan_days
  SET workout_id = cloned_workout_id, updated_at = now()
