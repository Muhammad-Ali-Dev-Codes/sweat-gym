-- 0041: Allow editing completed plan days for future repeat sessions.
-- The edit function still clones the workout, so existing sessions remain immutable.
...
BEGIN
  ...
  IF day_row.status NOT IN ('locked', 'available', 'completed') THEN
    RAISE EXCEPTION 'This plan day cannot be edited while a workout is in progress';
  END IF;
  ...
  LOOP
    IF NOT EXISTS (SELECT 1 FROM exercises WHERE id = (exercise_row->>'exerciseId')::UUID AND is_active = true) THEN
  ...
  VALUES (source_workout.name || ' (Edited)', 'edited-' || replace(p_plan_day_id::TEXT, '-', '') || '-' || substr(gen_random_uuid()::TEXT, 1, 8), source_workout.description, GREATEST(60, total_seconds), GREATEST(1, round(total_seconds / 60.0 * 8)), true, auth.uid())
