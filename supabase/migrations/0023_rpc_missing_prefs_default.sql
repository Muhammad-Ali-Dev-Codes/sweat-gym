-- Migration 0023 (Phase 11): Fix notification preference defaults in RPC
--
-- Live-testing defect (QA-H7): PL/pgSQL SELECT INTO assigns NULL to its
-- targets when the query matches zero rows. Users who never opened
-- notification settings therefore had no notification_preferences row,
-- which nulled v_allow_progress / v_allow_streaks and silently suppressed
-- every completion, streak, and milestone notification.
--
-- Fix: after the optional read, restore the documented defaults when no
-- preference row exists (NOT FOUND).
...
BEGIN
  ...
  -- Preferences. SELECT INTO nulls its targets when no row matches, so
  -- re-assert the enabled defaults whenever the member has no settings row.
  SELECT COALESCE(progress_updates, TRUE), COALESCE(streak_reminders, TRUE)
  INTO v_allow_progress, v_allow_streaks
  FROM notification_preferences
  WHERE user_id = v_user_id;
  IF NOT FOUND THEN
    v_allow_progress := TRUE;
    v_allow_streaks := TRUE;
  END IF;
...
COMMENT ON FUNCTION complete_workout_session_rpc IS 'Authoritative idempotent workout completion. Fails closed for anonymous callers, foreign owners, locked plan days; missing preference row counts as enabled.';
